...
interface NotifyTempFn {
    (actionType: string, obj: INotifyObj, effectObj?: INotifyObj ,comment?: Comment): string;
}
...
${effectObj.nickname}</a> 加入`;
        ...
        case "system_coupon":
            ...
            break;
        case "comment":
            content = `<a  href='https://lceda.cn/${obj.userName}' target='_blank' \
style='text-decoration: none; color:#58f;'>${obj.nickname}</a> 评论了工程 <a href='https://lceda.cn/lcedaxiaoju/${effectObj.name}' target='_blank' style='text-decoration: none; color:#58f;'>${effectObj.nickname}</a> :  ${comment.content}。`;
            break;
    }
    return content;
};
